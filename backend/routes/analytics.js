...
router.get('/', auth, checkRole(['Admin', 'Asset Manager']), async (req, res) => {
  try {
    ...
    const maintenance = await db.read('maintenance');
    const users = await db.read('users');

    const todayStr = new Date().toISOString().split('T')[0];

    const statusDistribution = assets.reduce((acc, a) => { acc[a.status] = (acc[a.status] || 0) + 1; return acc; }, {});
    ...
    const idleAssets = assets.filter(a => a.status === 'Available').map(a => ({ id: a.id, assetTag: a.assetTag, name: a.name, location: a.location }));
    ...
    const assetsNearRetirement = assets.filter(a => {
      ...
      return (acqYear + cat.expectedLife) - new Date().getFullYear() <= 1;
    }).map(a => ({ id: a.id, assetTag: a.assetTag, name: a.name, acquisitionDate: a.acquisitionDate }));

    const assetsDueMaintenance = maintenance
      .filter(t => t.status === 'Pending' || t.status === 'In Progress')
      .map(t => {
        const asset = assets.find(a => a.id === t.assetId);
        return { ticketId: t.id, assetTag: asset ? asset.assetTag : 'N/A', issue: t.issue, priority: t.priority, status: t.status };
      });

    const utilizationList = assets.map(a => ({
      id: a.id, assetTag: a.assetTag, name: a.name,
      usageScore: (a.history || []).filter(h => ['Allocated', 'Transferred', 'Reserved'].includes(h.eventType)).length + (a.status === 'Allocated' ? 2 : 0)
    }));

    const overdueReturns = assets.filter(a => a.status === 'Allocated' && a.expectedReturnDate && a.expectedReturnDate < todayStr).length;
    ...
    res.json({
      ...
      statusDistribution, departmentAllocation, categoryDistribution,
      maintenanceFrequency, bookingHeatmap, idleAssets, assetsNearRetirement,
      assetsDueMaintenance,
      mostUsedAssets: [...utilizationList].sort((a, b) => b.usageScore - a.usageScore).slice(0, 5),
      leastUsedAssets: [...utilizationList].sort((a, b) => a.usageScore - b.usageScore).slice(0, 5)
    });
  } catch (err) {
